...
| node-d (cliproxy-node-d) | i-0fd32ef2df7d69857 | c7g.large (2vCPU/4GB) | 3.19.6.233 | 172.31.24.86 | 已从 NLB 摘除并 stop，保留用于回滚 | 不承载新流量；停止前 auth 文件已迁出到 node-a/b/c |
...
2026-05-25 用本地 `biao-aws` profile 复核并执行过扩缩容维护：NLB target group 当前只保留 node-a/b/c，node-d 已 deregister 并 stop。后续正式扩缩容仍需用有权限的 AWS profile 复核 EC2 和 target health。
...
|------|------|---------|---------------------|------|
| Nginx | `0.0.0.0:443`, `[::]:443` | active: node-a/b/c；node-d stopped | `nginx.service` | TLS 终止、路径路由、来源访问控制 |
| LiteLLM | `127.0.0.1:4000` | active: node-a/b/c；node-d stopped | `litellm-proxy.service` → `/home/ec2-user/litellm-proxy/venv/bin/litellm --config /home/ec2-user/litellm-proxy/config.yaml --host 127.0.0.1 --port 4000 --telemetry False` | 用户 API key 校验、模型权限、预算、限流、审计和 SpendLogs |
| cliproxyapi | `127.0.0.1:8317` | active: node-a/b/c；node-d stopped | `cliproxyapi.service` → `/home/ec2-user/CLIProxyAPI/cliproxyapi` | Go 代理核心，上游适配、OAuth/provider/auth-file 管理，不再作为用户 key allowlist 的唯一入口 |
...
| Key Portal health-agent | `127.0.0.1:18081` | active: node-a/b/c；node-d stopped | `key-portal-health.service` → `/usr/bin/python3 /home/ec2-user/CLIProxyAPI/key-portal/health_agent.py` | NLB-facing 业务健康检查 |
...
其中 node-d 仍可能存在于 Key Portal 默认管理/监控节点列表，但 2026-05-25 已从 NLB 摘除并 stop；如果继续保留在 `CLIPROXY_NODES`/监控列表中，NLB monitor 会直接探测 node-d `/healthz` 并可能记录 node health event。node-e/node-f 是配置中预留或候选节点；2026-05-24 从 node-a 直接探测 `/healthz` 不可达，不能在未验证前视为 NLB active target。
...
2026-05-25 Node-D 下线维护时，按人工指令把 Node-D 上的可用 auth 文件迁出并在 active 节点间重新均衡；维护完成后 node-a/b/c 各有 7 个 usable auth files，node-d 停机且不承载新流量。
...
- node-d (`i-0fd32ef2df7d69857`) 已于 2026-05-25 deregister 并 stop
...
- 2026-05-25 Node-D 停机维护期间，`key-portal/data/alert_mute.json` 已设为 muted；解除静默前先移除/禁用 Node-D 的健康监控，或预期会产生 node-d health event。
