...
class StatusEventsService:
    ...
    def daily_usage_record_snapshot(self):
        ...
        window_end = datetime.strptime(today, "%Y-%m-%d") + timedelta(hours=20)
        window_start = window_end - timedelta(days=1)
        window_start_label = window_start.strftime("%Y-%m-%d %H:%M")
...
WITH windows AS (
    ...
    FROM daily
    WHERE window_end <> {window_end_sql}
...
SELECT json_build_object(
    'today', {self.sql_literal(today)},
    'window_start', {window_start_sql},
    'window_end', {window_end_sql},
    ...
    'previous_record_window_start', (SELECT window_start FROM record_row),
    'previous_record_window_end', (SELECT window_end FROM record_row),
...
"""
        ...
        if not payload:
            return {
                ...
                "previous_record_tokens": 0,
                "previous_record_requests": 0,
                "source": "litellm_spendlogs",
                "error": "litellm spend query failed",
            }
        return payload
...
"""
    ...
    def check_daily_usage_record(self):
        ...
        today = snapshot.get("today") or self.beijing_today()
        window_start = snapshot.get("window_start") or "-"
        window_end = snapshot.get("window_end") or "-"
        previous_date = snapshot.get("previous_record_date") or "-"
        previous_window_start = snapshot.get("previous_record_window_start") or "-"
        previous_window_end = snapshot.get("previous_record_window_end") or "-"
        ...
        saved = self.portal_state.upsert_status_event(
            ...
            severity="info",
            title="今日用量突破历史新高",
            summary=f"今日 Tokens {self.format_compact_number(today_tokens)}，超过历史最高 {self.format_compact_number(record_tokens)}。",
            reason="20:00 daily token record",
            affected_nodes=[],
        ...
        if saved.get("created"):
            ...
            content = "\n".join([
                "**事件**: 今日用量突破历史新高",
                f"**统计窗口**: {window_start} - {window_end}（北京时间）",
                f"**今日 Tokens**: {self.format_compact_number(today_tokens)}",
                f"**历史最高**: {self.format_compact_number(record_tokens)} ({previous_window_start} - {previous_window_end})",
